"""Static architecture checks for VIA Lote 8B.3B."""
...
def test_mcda_policy_has_no_forbidden_imports() -> None:
    forbidden_prefixes = (
        "fastapi",
        "sqlalchemy",
        "via.config",
    ...
    offenders: list[str] = []
    for imported_name in _imports_from(DOMAIN / "mcda_policy.py"):
        if ".infrastructure" in imported_name or ".interfaces" in imported_name:
            offenders.append(f"mcda_policy.py imports {imported_name}")
...
def test_lote8b3b_does_not_add_infrastructure_http_or_external_calls() -> None:
    forbidden_tokens = (
        "Outbox",
        "Session",
        "create_engine",
    ...
    text = (DOMAIN / "mcda_policy.py").read_text(encoding="utf-8")

    assert [token for token in forbidden_tokens if token in text] == []


def test_lote8b3b_does_not_implement_persistence_events_or_recommendation() -> None:
    forbidden_tokens = (
        "Repository",
        "OutboxWriter",
